...
flowchart LR
    ...
    E --> F[Optional metadata filter]
    F --> G[Ranked results]
```

That is the pipeline I keep in my head: first turn the question into a vector, then let the ANN index narrow the search space, then decide which few candidates deserve to survive. The metadata filter is optional in theory, but in any real product I treat tenant, language, or document version as part of retrieval quality, not a nice-to-have.
...
My decision rule is simple: choose vector search when people ask the same thing in many different words, and stay with plain keyword search when the wording is stable and the corpus is small enough to inspect by hand.
