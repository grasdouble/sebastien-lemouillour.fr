...
My rule is simple: if a human should check a source before answering, the model should check a source too. Keep prompt-only for demos, tiny static references, or one-off tasks where you hand-curate the context. The moment the answer needs to survive changing documents or user trust, move to retrieval, then continue with semantic similarity, the technique that helps a system find text with the same meaning even when the wording changes.
