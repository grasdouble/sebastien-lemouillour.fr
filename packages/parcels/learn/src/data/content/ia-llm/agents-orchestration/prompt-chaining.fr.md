...
```mermaid
flowchart TD
  A["Prompt 1 : classifier le ticket"] --> B["Sortie 1 : intention en JSON"]
  B --> C{"Contrat valide ?"}
  C -- Non --> D["Échec typé / nouvel essai"]
  C -- Oui --> E["Prompt 2 : récupérer le contexte\n(avec ticket + intention)"]
  E --> F["Sortie 2 : contexte"]
  F --> G["Délimiter et traiter comme\nun contexte non fiable"]
  G --> H["Prompt 3 : rédiger la réponse\n(avec ticket + intention + contexte)"]
  H --> I["Sortie 3 : brouillon"]
  I --> J["Prompt 4 : valider la réponse\n(ton, règles, faits manquants)"]
  J --> K["Sortie finale"]
```
