...
So if a document is long, I would not drop the whole thing into the chat unless I had no better option. I would keep the task first, quote the exact passage that matters, and trim everything that does not change the answer. My rule is simple: if the answer depends on one passage, make that passage obvious, and if the prompt contains several goals at once, split it before you send it.
