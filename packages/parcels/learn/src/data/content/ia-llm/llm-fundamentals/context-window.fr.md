...
Donc, si un document est long, je ne le collerais pas entier dans le chat sauf si je n'ai vraiment pas mieux. Je garderais la tâche en premier, je citerais exactement le passage décisif, et je couperais tout ce qui ne change pas la réponse. Ma règle est simple : si la réponse dépend d'un passage précis, rendez ce passage impossible à manquer, et si le prompt poursuit plusieurs objectifs à la fois, découpez-le avant de l'envoyer.
