...
Before I argue about tooling, I like one table that makes the slices non-negotiable.

| Evaluation slice | Metric                                             | Threshold | Action on fail                                                                     |
| ---------------- | -------------------------------------------------- | --------- | ---------------------------------------------------------------------------------- |
| Factuality       | Grounded answer accuracy on an incident-backed set | ≥ 0.95    | Block prompt or model changes and add the failing case to the regression set       |
| Safety           | Harmful-request refusal pass rate                  | = 1.00    | Fail the pipeline and require manual review before any deploy                      |
| Coherence        | Rubric or judge pass rate on multi-turn responses  | ≥ 0.90    | Hold release, inspect traces, and tighten prompt or context assembly               |
| Latency          | End-to-end p95 on representative tasks             | ≤ 4 s     | Reduce tool fan-out, shrink context, or move heavy work to async flows             |
| Cost             | Cost per successful task                           | ≤ €0.35   | Stop rollout, trim tokens, and rerun with a cheaper model or shorter output policy |
...
The release path should look boring on purpose.

```mermaid
flowchart LR
    pr[Pull request] --> suite[Eval suite]
    suite --> thresholds{Threshold check}
    thresholds -- fail --> blocked[Merge blocked]
    thresholds -- pass --> gate[Merge gate]
    gate --> staging[Staging eval]
    staging --> ready{Staging passes}
    ready -- fail --> hold[Release held]
    ready -- pass --> prod[Production release]
```
